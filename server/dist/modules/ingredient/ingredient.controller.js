"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.IngredientController = void 0;
const ingredient_service_1 = require("./ingredient.service");
class IngredientController {
    static async list(req, res) {
        try {
            const { familyId } = req.params;
            const ingredients = await ingredient_service_1.IngredientService.listByFamily(familyId);
            res.json(ingredients);
        }
        catch (err) {
            res.status(500).json({ error: err.message });
        }
    }
    static async create(req, res) {
        try {
            const { familyId } = req.params;
            const { name, storageType, quantity, unit } = req.body;
            if (!name || !storageType) {
                return res.status(400).json({ error: 'Name and storageType required' });
            }
            const ingredient = await ingredient_service_1.IngredientService.create({
                familyId,
                name,
                storageType,
                quantity: quantity || 1,
                unit: unit || '个'
            });
            res.status(201).json(ingredient);
        }
        catch (err) {
            res.status(500).json({ error: err.message });
        }
    }
    static async update(req, res) {
        try {
            const { id } = req.params;
            const updated = await ingredient_service_1.IngredientService.update(id, req.body);
            if (!updated) {
                return res.status(404).json({ error: 'Ingredient not found' });
            }
            res.json(updated);
        }
        catch (err) {
            res.status(500).json({ error: err.message });
        }
    }
    static async delete(req, res) {
        try {
            const { id } = req.params;
            const success = await ingredient_service_1.IngredientService.delete(id);
            if (!success) {
                return res.status(404).json({ error: 'Ingredient not found' });
            }
            res.json({ success: true });
        }
        catch (err) {
            res.status(500).json({ error: err.message });
        }
    }
}
exports.IngredientController = IngredientController;
